...
import { z } from 'zod';
import { dealQueries, weightHistoryQueries } from '../models/queries.ts';
...
router.get('/history/all', asyncHandler(async (req, res) => {
    const history = await weightHistoryQueries.getAll();
    res.json(history);
}));
...
router.get('/:id/history', asyncHandler(async (req, res) => {
    const history = await weightHistoryQueries.getByDealId(String(req.params.id));
    res.json(history);
}));
