...
import { Users, Factory, IndianRupee, Search, Eye, Download, MessageCircle, Wallet } from "lucide-react";
...
type LedgerTab = "customer" | "supplier" | "cash" | "bank";
...
function LedgerPage() {
  ...
  const parties = (tab === "customer" ? customers : suppliers)
    .filter((p) => p.name.toLowerCase().includes(q.toLowerCase()));

  // Customer / Supplier running lines
  function lines(name: string, side: Side): LedgerLine[] {
    const out: LedgerLine[] = [];
    if (side === "customer") {
      sales.filter((s) => s.party === name).forEach((s) => out.push({
        date: s.date, particulars: "Sales Invoice", ref: s.no, debit: Number(s.amount || 0), credit: 0,
      }));
      payments.filter((p) => p.party === name && p.direction === "In").forEach((p) => out.push({
        date: p.date, particulars: `Receipt (${p.mode})`, ref: p.no, debit: 0, credit: Number(p.amount || 0),
      }));
    } else {
      purchases.filter((s) => s.party === name).forEach((s) => out.push({
        date: s.date, particulars: "Purchase Bill", ref: s.no, debit: 0, credit: Number(s.amount || 0),
      }));
      payments.filter((p) => p.party === name && p.direction === "Out").forEach((p) => out.push({
        date: p.date, particulars: `Payment (${p.mode})`, ref: p.no, debit: Number(p.amount || 0), credit: 0,
      }));
    }
  ...
  // Cash Ledger Lines
  const cashLines = useMemo((): LedgerLine[] => {
    const out: LedgerLine[] = [];
    payments.filter((p) => p.mode === "Cash" && p.direction === "In").forEach((p) => out.push({
      date: p.date, particulars: `Cash Receipt: ${p.party}`, ref: p.no, debit: Number(p.amount || 0), credit: 0,
    }));
    payments.filter((p) => p.mode === "Cash" && p.direction === "Out").forEach((p) => out.push({
      date: p.date, particulars: `Cash Payment: ${p.party}`, ref: p.no, debit: 0, credit: Number(p.amount || 0),
    }));
    trips.forEach((t) => {
      if (Number(t.expense || 0) > 0) out.push({
        date: t.date, particulars: `Trip Expense: ${t.tripNo} (${t.driver})`, ref: t.tripNo, debit: 0, credit: Number(t.expense || 0),
      });
    });
    expenses.filter((e) => e.mode === "Cash").forEach((e) => out.push({
      date: e.date, particulars: `Cash Expense: ${e.category} (${e.paidTo})`, ref: e.no, debit: 0, credit: Number(e.amount || 0),
    }));
    return out.sort((a, b) => a.date.localeCompare(b.date));
  }, [payments, trips, expenses]);

  // Bank Ledger Lines
  const bankLines = useMemo((): LedgerLine[] => {
    const out: LedgerLine[] = [];
    payments.filter((p) => p.mode !== "Cash" && p.direction === "In").forEach((p) => out.push({
      date: p.date, particulars: `Bank Receipt (${p.mode}): ${p.party}`, ref: p.no, debit: Number(p.amount || 0), credit: 0,
    }));
    payments.filter((p) => p.mode !== "Cash" && p.direction === "Out").forEach((p) => out.push({
      date: p.date, particulars: `Bank Payment (${p.mode}): ${p.party}`, ref: p.no, debit: 0, credit: Number(p.amount || 0),
    }));
    expenses.filter((e) => e.mode !== "Cash").forEach((e) => out.push({
      date: e.date, particulars: `Bank Expense (${e.mode}): ${e.category} (${e.paidTo})`, ref: e.no, debit: 0, credit: Number(e.amount || 0),
    }));
    return out.sort((a, b) => a.date.localeCompare(b.date));
  }, [payments, expenses]);
  ...
  function balanceFor(name: string, s: Side): number {
    ...
    const seed = Number(seedRow?.outstanding || 0);
  ...
  function excel(name: string, s: Side) {
    const ls = lines(name, s);
    let bal = Number((s === "customer" ? customers : suppliers).find((p) => p.name === name)?.outstanding || 0);
    const body = ls.map((l) => {
      bal += s === "customer" ? l.debit - l.credit : l.credit - l.debit;
      return [l.date, l.particulars, l.ref, l.debit || 0, l.credit || 0, bal];
    });
    exportExcel(
      `Ledger — ${name}`,
      ["Date", "Particulars", "Ref", "Debit", "Credit", "Balance"],
      body.length ? body : [["—", "Opening balance carried forward", "—", 0, 0, bal]],
      [{ label: "Closing balance", value: inr(bal) }]
    );
  }

  function exportAccountPdf(name: string, entries: LedgerLine[]) {
    let bal = 0;
    ...
    generatePdf({
      title: `${name} Ledger`,
      subtitle: `Account statement as of ${new Date().toLocaleDateString("en-IN")}`,
      filename: `${name.toLowerCase()}-ledger-${Date.now()}.pdf`,
      head: ["Date", "Particulars", "Ref", "Debit", "Credit", "Balance"],
      body: body.length ? body : [["—", "Opening Balance", "—", "—", "—", inr(0)]],
      totals: [{ label: "Closing Balance", value: inr(bal) }],
    });
  }

  function exportAccountExcel(name: string, entries: LedgerLine[]) {
    let bal = 0;
    const body = entries.map((l) => {
      bal += l.debit - l.credit;
      return [l.date, l.particulars, l.ref, l.debit || 0, l.credit || 0, bal];
    ...
    exportExcel(
      `${name} Ledger`,
      ["Date", "Particulars", "Ref", "Debit", "Credit", "Balance"],
      body.length ? body : [["—", "Opening Balance", "—", 0, 0, 0]],
      [{ label: "Closing Balance", value: inr(bal) }]
    );
  }
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 px-6 pt-6 md:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard label="Cash Balance" value={inr(cashBalance)} hint="In-hand cash" icon={Wallet} tone={cashBalance >= 0 ? "primary" : "destructive"} />
        <StatCard label="Bank Balance" value={inr(bankBalance)} hint="Bank / UPI / Cheque" icon={Wallet} tone={bankBalance >= 0 ? "primary" : "destructive"} />
      </div>
        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as LedgerTab)}>
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-3">
            <TabsList>
              ...
              <TabsTrigger value="cash"><Wallet className="mr-1 h-3.5 w-3.5" />Cash Ledger</TabsTrigger>
              <TabsTrigger value="bank"><Wallet className="mr-1 h-3.5 w-3.5" />Bank Ledger</TabsTrigger>
            </TabsList>
            
            {(tab === "customer" || tab === "supplier") && (
              <div className="relative w-full max-w-xs">
                <Search className="pointer-events-none absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" />
                <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search party…" className="h-9 pl-8 bg-background" />
              </div>
            )}
          </div>
                ...
                return (
                  <div key={p.id} className="rounded-xl border border-border bg-card p-4 shadow-sm">
                    ...
                    <div className="mt-3 grid grid-cols-3 gap-1">
                      <Button size="sm" variant="ghost" className="h-8 text-[11px]" onClick={() => setView({ name: p.name, side: "customer" })}>
                        <Eye className="mr-1 h-3 w-3" />Open
                      </Button>
                      <Button size="sm" variant="ghost" className="h-8 text-[11px]" onClick={() => wa(p.name, "customer")}>
                        <MessageCircle className="mr-1 h-3 w-3" />Remind
                      </Button>
                      <Button size="sm" variant="default" className="h-8 text-[11px]" onClick={() => setPayOpen({ name: p.name, side: "customer", amount: bal })}>
                        <IndianRupee className="mr-1 h-3 w-3" />Receive
                      </Button>
                    </div>
                  </div>
                );
              ...
              {parties.map((p) => {
                ...
                return (
                  <div key={p.id} className="rounded-xl border border-border bg-card p-4 shadow-sm">
                    ...
                    <div className="mt-3 rounded-md bg-muted/40 p-3 text-center">
                      <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Payable</p>
                      <p className={`font-display text-lg font-semibold ${bal > 0 ? "text-warning" : "text-success"}`}>{inr(bal)}</p>
                    </div>
                    <div className="mt-3 grid grid-cols-3 gap-1">
                      <Button size="sm" variant="ghost" className="h-8 text-[11px]" onClick={() => setView({ name: p.name, side: "supplier" })}>
                        <Eye className="mr-1 h-3 w-3" />Open
                      </Button>
                      <Button size="sm" variant="ghost" className="h-8 text-[11px]" onClick={() => wa(p.name, "supplier")}>
                        <MessageCircle className="mr-1 h-3 w-3" />Remind
                      </Button>
                      <Button size="sm" variant="default" className="h-8 text-[11px]" onClick={() => setPayOpen({ name: p.name, side: "supplier", amount: bal })}>
          ...
          <TabsContent value="cash" className="mt-4 space-y-4">
            <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="font-display font-semibold text-sm">Cash Ledger Running Statement</h3>
                  <p className="text-xs text-muted-foreground">Transactions handled in cash mode</p>
                </div>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => exportAccountExcel("Cash", cashLines)}>
                    <Download className="h-3.5 w-3.5 mr-1" />
                    Export Excel
                  </Button>
                  <Button variant="outline" size="sm" onClick={() => exportAccountPdf("Cash", cashLines)}>
                    <Download className="h-3.5 w-3.5 mr-1" />
                    Export PDF
                  </Button>
                </div>
              </div>
              <LedgerTable name="Cash" side="customer" compute={() => cashLines} seed={0} />
            </div>
          </TabsContent>
          ...
          <TabsContent value="bank" className="mt-4 space-y-4">
            <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="font-display font-semibold text-sm">Bank / UPI / Cheque Ledger Running Statement</h3>
                  <p className="text-xs text-muted-foreground">Transactions handled via bank transfers and digital payments</p>
                </div>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => exportAccountExcel("Bank", bankLines)}>
                    <Download className="h-3.5 w-3.5 mr-1" />
                    Export Excel
                  </Button>
                  <Button variant="outline" size="sm" onClick={() => exportAccountPdf("Bank", bankLines)}>
                    <Download className="h-3.5 w-3.5 mr-1" />
                    Export PDF
                  </Button>
                </div>
              </div>
              <LedgerTable name="Bank" side="customer" compute={() => bankLines} seed={0} />
            </div>
          </TabsContent>
        </Tabs>
      </div>
      ...
      <Dialog open={!!view} onOpenChange={(v) => !v && setView(null)}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl">
          ...
          <DialogFooter>
            ...
            <Button variant="outline" onClick={() => view && excel(view.name, view.side)}><Download className="mr-1 h-4 w-4" />Export Excel</Button>
            <Button onClick={() => view && pdf(view.name, view.side)}><Download className="mr-1 h-4 w-4" />Download PDF</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      <PaymentDialog open={!!payOpen} onClose={() => setPayOpen(null)} target={payOpen} onSubmit={recordPayment} loading={isPaymentLoading} />
    </div>
  );
}
      ...
      <table className="w-full text-xs">
        ...
        <tbody>
          <tr className="border-t border-border bg-muted/20">
            <td className="p-2 italic text-muted-foreground" colSpan={5}>Opening balance</td>
            ...
            ls.map((l, i) => {
              ...
              return (
                <tr key={i} className="border-t border-border">
                  ...
                  <td className="p-2 text-right tabular-nums text-success">{l.debit ? inr(l.debit) : "—"}</td>
                  <td className="p-2 text-right tabular-nums text-warning">{l.credit ? inr(l.credit) : "—"}</td>
                  <td className="p-2 text-right font-medium tabular-nums">{inr(bal)}</td>
                </tr>
              );
            })
...
function PaymentDialog({ open, onClose, target, onSubmit, loading }: { open: boolean; onClose: () => void; target: { name: string; side: Side; amount: number } | null; onSubmit: (f: { date: string; amount: number; mode: "Cash" | "Bank" | "UPI" | "Cheque"; reference: string; note: string }) => void; loading: boolean }) {
  const [f, setF] = useState({ date: getLocalDateString(), amount: 0, mode: "Bank" as const, reference: "", note: "" });
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Date"><Input type="date" value={f.date} onChange={(e) => setF({ ...f, date: e.target.value })} disabled={loading} /></Field>
          <Field label="Amount"><Input type="number" value={f.amount || ""} onChange={(e) => setF({ ...f, amount: Number(e.target.value) })} disabled={loading} /></Field>
          <Field label="Mode">
            <select className="h-9 rounded-md border border-input bg-background px-2 text-sm focus:ring-1 focus:ring-ring" value={f.mode} onChange={(e) => setF({ ...f, mode: e.target.value as typeof f.mode })} disabled={loading}>
              <option>Cash</option><option>Bank</option><option>UPI</option><option>Cheque</option>
            </select>
          </Field>
          <Field label="Reference (UTR / Cheque)"><Input value={f.reference} onChange={(e) => setF({ ...f, reference: e.target.value })} disabled={loading} /></Field>
          <Field label="Note" full><Input value={f.note} onChange={(e) => setF({ ...f, note: e.target.value })} placeholder="Optional remark" disabled={loading} /></Field>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={onClose} disabled={loading}>Cancel</Button>
          <Button onClick={() => f.amount > 0 ? onSubmit(f) : toast.error("Enter amount")} disabled={loading}>
            {loading ? "Saving..." : "Save"}
          </Button>
        </DialogFooter>
      </DialogContent>
